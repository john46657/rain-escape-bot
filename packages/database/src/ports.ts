 ...
 */
import type {
  AchievementEntity, ApiKeyEntity, AnalyticsSnapshotEntity, AuditLogEntity, AutomationEntity,
...
import type {
  IncidentSeverity, IncidentStatus, Locale, ModerationAction, PremiumTier,
  TicketCategory, TicketStatus, TransactionType,
} from '@nexus/shared';
...
export interface UserRepository {
  upsertFromDiscord(input: {
    discordId: string; username: string; globalName?: string | null; avatar?: string | null;
  }): Promise<UserEntity>;
...
export interface GuildRepository {
  upsert(input: {
    discordId: string; name: string; icon?: string | null; ownerId: string; memberCount?: number;
  }): Promise<GuildEntity>;
...
export interface ModerationRepository {
  ...
  listCases(guildId: string, query?: PageQuery & {
    targetId?: string; moderatorId?: string; action?: ModerationAction;
  }): Promise<Page<ModerationCaseEntity>>;
  updateCase(id: string, patch: Partial<Pick<ModerationCaseEntity,
    'reason' | 'active' | 'revoked' | 'revokedById' | 'revokedAt' | 'logMessageId'>>): Promise<void>;
  ...
  addWarning(input: {
    guildId: string; targetId: string; moderatorId: string; reason: string;
    points?: number; caseRef?: string | null; expiresAt?: Date | null;
...
export interface SecurityRepository {
  createIncident(input: Omit<SecurityIncidentEntity, 'id' | 'createdAt' | 'updatedAt' | 'acknowledgedBy' | 'resolvedBy'> &
    Partial<Pick<SecurityIncidentEntity, 'acknowledgedBy' | 'resolvedBy'>>): Promise<SecurityIncidentEntity>;
  listIncidents(query?: PageQuery & {
    guildId?: string; status?: IncidentStatus; severity?: IncidentSeverity;
  }): Promise<Page<SecurityIncidentEntity>>;
  getIncident(id: string): Promise<SecurityIncidentEntity | null>;
  updateIncident(id: string, patch: Partial<Pick<SecurityIncidentEntity,
    'status' | 'severity' | 'acknowledgedBy' | 'resolvedBy' | 'actionsTaken'>>): Promise<void>;
}

export interface AuditRepository {
  log(input: Omit<AuditLogEntity, 'id' | 'createdAt'>): Promise<AuditLogEntity>;
  list(query?: PageQuery & { guildId?: string; actorId?: string; action?: string }): Promise<Page<AuditLogEntity>>;
}

export interface TicketRepository {
  create(input: {
    guildId: string; panelId?: string | null; channelId: string; openerId: string;
    category: TicketCategory; subject?: string | null; formData?: Record<string, unknown>;
  ...
  list(guildId: string, query?: PageQuery & { status?: TicketStatus; openerId?: string }): Promise<Page<TicketEntity>>;
...
export interface LevelRepository {
  ...
  addXp(guildId: string, userId: string, amount: number): Promise<{ profile: XPProfileEntity; leveledUp: boolean; previousLevel: number }>;
  setLevel(guildId: string, userId: string, level: number): Promise<XPProfileEntity>;
  leaderboard(guildId: string, limit?: number, offset?: number): Promise<Array<XPProfileEntity & { rank: number }>>;
  rankOf(guildId: string, userId: string): Promise<number>;
  listRewards(guildId: string): Promise<Array<{ level: number; roleId: string | null; coins: number; removePrevious: boolean }>>;
}
...
export interface EconomyRepository {
   ...
   */
  mutate(mutation: EconomyMutation): Promise<{ profile: EconomyProfileEntity; transaction: TransactionEntity | null }>;
  ...
  touchCooldown(guildId: string, userId: string, field: 'lastDailyAt' | 'lastWeeklyAt' | 'lastWorkAt' | 'lastCrimeAt' | 'lastRobAt', at: Date, streakDays?: number): Promise<void>;
...
export interface VerificationRepository {
  create(input: {
    userId: string; discordId: string; codeHash: string; codeHint: string;
    guildId?: string | null; gameId?: string | null; expiresAt: Date;
...
export interface RobloxRepository {
  linkAccount(input: {
    userId: string; discordId: string; robloxUserId: string; username: string;
    displayName?: string | null; avatarUrl?: string | null; method?: 'game' | 'oauth' | 'manual';
  ...
  recordHeartbeat(input: Omit<RobloxServerEntity, 'id' | 'startedAt' | 'status'> & { startedAt?: Date }): Promise<RobloxServerEntity>;
  ...
  recordEvent(input: Omit<RobloxEventEntity, 'id' | 'receivedAt' | 'processedAt' | 'error'>): Promise<{ event: RobloxEventEntity; duplicate: boolean }>;
  ...
  listEventRoutes(guildId: string, eventType: string): Promise<Array<{ channelId: string; gameId: string | null; filter: Record<string, unknown> }>>;
  upsertEventRoute(input: { guildId: string; gameId: string | null; eventType: string; channelId: string; enabled: boolean }): Promise<void>;

  queueCommand(input: Omit<RobloxCommandEntity, 'id' | 'status' | 'deliveredAt' | 'acknowledgedAt' | 'result' | 'error' | 'createdAt'>): Promise<RobloxCommandEntity>;
  ...
  startSession(input: { gameId: string; robloxUserId: string; jobId: string | null; accountId?: string | null }): Promise<void>;
...
export interface RewardRepository {
  ...
  grant(input: Omit<RewardGrantEntity, 'id' | 'createdAt' | 'status'> & { status?: RewardGrantEntity['status'] }): Promise<{ grant: RewardGrantEntity; created: boolean }>;
...
export interface GiveawayRepository {
  create(input: Omit<GiveawayEntity, 'id' | 'createdAt' | 'ended' | 'entries' | 'winners' | 'rerollCount'>): Promise<GiveawayEntity>;
...
export interface AutomationRepository {
  ...
  create(input: Omit<AutomationEntity, 'id' | 'runCount' | 'errorCount' | 'lastRunAt' | 'lastError'>): Promise<AutomationEntity>;
  update(id: string, patch: Partial<AutomationEntity>): Promise<void>;
  delete(id: string): Promise<boolean>;
  recordRun(id: string, status: 'SUCCESS' | 'SKIPPED' | 'FAILED', trace: Record<string, unknown>, durationMs: number): Promise<void>;
}

export interface BackupRepository {
  create(input: { guildId: string; name: string; createdById: string; payload: unknown; summary: Record<string, number> }): Promise<BackupEntity>;
...
export interface ApiKeyRepository {
  create(input: Omit<ApiKeyEntity, 'id' | 'createdAt' | 'lastUsedAt' | 'usageCount' | 'revokedAt'>): Promise<ApiKeyEntity>;
  findByHash(hash: string): Promise<ApiKeyEntity | null>;
  list(guildId: string): Promise<ApiKeyEntity[]>;
  revoke(id: string): Promise<void>;
  recordUsage(id: string, entry: { method: string; path: string; statusCode: number; durationMs: number; ip?: string; requestId?: string; error?: string }): Promise<void>;
  listRequests(apiKeyId: string, limit?: number): Promise<Array<{ method: string; path: string; statusCode: number; durationMs: number; createdAt: Date }>>;
}

export interface NotificationRepository {
  create(input: Omit<NotificationEntity, 'id' | 'createdAt' | 'read'>): Promise<NotificationEntity>;
  list(query: { guildId?: string; userId?: string; unreadOnly?: boolean; limit?: number }): Promise<NotificationEntity[]>;
  markRead(id: string): Promise<void>;
}

export interface AnalyticsRepository {
  record(snapshot: AnalyticsSnapshotEntity): Promise<void>;
  query(guildId: string, scope: AnalyticsSnapshotEntity['scope'], from: Date, to: Date, granularity?: 'hour' | 'day'): Promise<AnalyticsSnapshotEntity[]>;
