      ...
      <div className="grid gap-6 lg:grid-cols-2">
          ...
          {boards && boards.levels.length > 0 ? (
            <ol className="space-y-2">
              {boards.levels.map((entry) => (
                <li key={entry.userId} className="flex items-center gap-3 rounded-lg bg-base-850/50 px-3 py-2">
            ...
            <EmptyState title="Keine XP-Daten" description="Sobald Mitglieder schreiben, entsteht hier eine Rangliste." />
          )}
        </Card>
        ...
        <Card title="Vermoegensrangliste">
          {boards && boards.economy.length > 0 ? (
            ...
            <EmptyState title="Keine Wirtschaftsdaten" description="Mit /daily, /work und dem Shop entstehen hier Werte." />
          )}
        </Card>
      </div>
    </div>
